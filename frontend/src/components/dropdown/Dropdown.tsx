import { Fragment, ButtonHTMLAttributes, PropsWithChildren, ReactNode } from 'react';
...
const Dropdown = ({ label, disabled, upPlacement, className, noArrow, defaultClasses = true, menuMiddle, menuClasses, children, svg, preLabel, id, ...props }: DropdownProps) => {
  return (
    <div className='dropdown relative inline-block'>
      <Menu as="div" {...props}>
        <Menu.Button disabled={disabled} id={id} className={`${defaultClasses ? 'select-none text-gray-900 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 whitespace-nowrap focus:ring-blue-300 disabled:hover:bg-white dark:bg-gray-600 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-700 dark:focus:ring-gray-700 group flex h-min items-center justify-center text-center font-medium focus:z-10 rounded-lg text-sm p-2' : ''} ${disabled ? '!cursor-not-allowed opacity-50' : ''} ${className ? className : ''}`}>
          {preLabel}
          {svg}
          {label}
        ...
        >
          <Menu.Items className={`${upPlacement ? 'bottom-[50px] origin-bottom' : ''} ${menuClasses} absolute flex flex-col mt-2 w-max rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50 select-none ${menuMiddle ? 'left-[50%] translate-x-[-50%]' : ''}`}>
            {children}
          </Menu.Items>
        </Transition>
      </Menu>
